...
# Specify the Evaluation language dict
...
train_source = os.path.join(temp_dir_path, "zs_s.train")
train_target = os.path.join(temp_dir_path, "zs_t.train")
test_source = os.path.join(temp_dir_path, "zs_s.test")
test_target = os.path.join(temp_dir_path, "zs_t.test")
dev_source = os.path.join(temp_dir_path, "zs_s.dev")
dev_target = os.path.join(temp_dir_path, "zs_t.dev")
...
obj = MultilingualAlignedCorpusReader(corpus_path=data_path, lang_dict=zs_train_lang_dict, target_token=True,
                                      eval_lang_dict=zs_eval_lang_dict, zero_shot=zero_shot, bilingual=bilingual)
...
if tokenization:
    ...
    for lang in zs_train_lang_dict['target']:

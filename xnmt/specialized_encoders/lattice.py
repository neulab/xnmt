...
from xnmt import batchers, events, input_readers, param_collections, param_initializers, sent, vocabs
...
class Lattice(sent.Sentence):
  ...
  """
  def __init__(self, nodes: Sequence[LatticeNode]):
    self.nodes = nodes
...
class LatticeTextReader(input_readers.BaseTextReader, Serializable):
  ...
  def __init__(self, vocab=None, use_words=True, use_chars=False, use_pronun_from=None):
    ...
    if vocab is not None:
      self.vocab.freeze()
      self.vocab.set_unk(vocabs.Vocab.UNK_STR)
...
class LatticeEmbedder(embedders.SimpleWordEmbedder, Serializable):
  ...
  def __init__(self,
               ...
               src_reader = Ref("model.src_reader", required=False),
               trg_reader = Ref("model.trg_reader", required=False)):

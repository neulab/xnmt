...
class MlpAttender(Attender, Serializable):
  ...
  def calc_attention(self, state):
    ...
    if self.truncate_dec_batches:
      if curr_sent_mask: state, WI, curr_sent_mask = batching.truncate_batches(state, WI, curr_sent_mask)
      else: state, WI = batching.truncate_batches(state, WI)
  ...
  def calc_context(self, state):
    attention = self.calc_attention(state)
    I = self.curr_sent.as_tensor()
    if self.truncate_dec_batches: I, attention = batching.truncate_batches(I, attention)
    return I * attention

  ...
  def transduce(self, expr_seq):
    ...
    for pos_i in range(seq_len):
      ...
      if expr_seq[0].mask is None:
        ...
        mask_expr = dy.inputTensor(np.expand_dims(expr_seq[0].mask.transpose(), axis=1), batched=True)
        inv_mask_expr = dy.inputTensor(1.0 - np.expand_dims(expr_seq[0].mask.transpose(), axis=1), batched=True)

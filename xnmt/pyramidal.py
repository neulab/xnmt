...
class PyramidalLSTMSeqTransducer(transducer.SeqTransducer, Serializable):
  """
  ...
  def __init__(self, layers=1,
    ...
    f = lstm.UniLSTMSeqTransducer(input_dim=input_dim, hidden_dim=hidden_dim / 2, dropout=dropout)
    b = lstm.UniLSTMSeqTransducer(input_dim=input_dim, hidden_dim=hidden_dim / 2, dropout=dropout)
    self.builder_layers.append((f, b))
    for _ in range(layers - 1):
      layer_input_dim = hidden_dim if downsampling_method=="skip" else hidden_dim*reduce_factor
      f = lstm.UniLSTMSeqTransducer(input_dim=layer_input_dim, hidden_dim=hidden_dim / 2, dropout=dropout)
  ...
  def __call__(self, es):
    ...
    for layer_i, (fb, bb) in enumerate(self.builder_layers):
      ...
      fs = fb(es_list)
      bs = bb([expression_sequence.ReversedExpressionSequence(es_item) for es_item in es_list])
      if layer_i < len(self.builder_layers) - 1:
        if self.downsampling_method=="skip":
          es_list = [expression_sequence.ExpressionSequence(expr_list=fs[::reduce_factor], mask=mask_out), expression_sequence.ExpressionSequence(expr_list=bs[::reduce_factor][::-1], mask=mask_out)]
          ...
          es_list = [expression_sequence.ExpressionSequence(expr_list=es_list_fwd[j], mask=mask_out) for j in range(reduce_factor)] + [expression_sequence.ExpressionSequence(expr_list=es_list_bwd[j], mask=mask_out) for j in range(reduce_factor)]
        else:
          raise RuntimeError("unknown downsampling_method %s" % self.downsampling_method)
      else:
        # concat final outputs
        ret_es = expression_sequence.ExpressionSequence(expr_list=[dy.concatenate([f, b]) for f, b in zip(fs, expression_sequence.ReversedExpressionSequence(bs))], mask=mask_out)

    self._final_states = [transducer.FinalTransducerState(dy.concatenate([fb.get_final_states()[0].main_expr(),

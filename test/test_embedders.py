...
class PretrainedSimpleWordEmbedderSanityTest(unittest.TestCase):
  def setUp(self):
    events.clear()
    self.input_reader = PlainTextReader(vocab=Vocab(vocab_file="test/data/head.ja.vocab"))
    list(self.input_reader.read_sents('test/data/head.ja'))
  ...
  def test_load(self):
    ...
    """
    embedder = PretrainedSimpleWordEmbedder(filename='test/data/wiki.ja.vec.small', emb_dim=300, vocab=self.input_reader.vocab)
    # self.assertEqual(embedder.embeddings.shape()[::-1], (self.input_reader.vocab_size(), 300))

    with open('test/data/wiki.ja.vec.small', encoding='utf-8') as vecfile:
